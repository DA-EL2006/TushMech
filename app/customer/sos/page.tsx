...
export default function SOSPage() {
  ...
  const [responder, setResponder] = useState<typeof RESPONDERS[0] | null>(null);
    ...
    try {
      ...
      if (raw) {
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          setCarName(`${parsed[0].year} ${parsed[0].make} ${parsed[0].model}`.trim());
        }
      }
    } catch {}
  }, []);
  ...
  const handleSOS = () => {
    ...
    setTimeout(() => {
      setResponder(RESPONDERS[0]);
      setPhase("matched");
      setTimeout(() => {
        setPhase("enroute");
        let e = RESPONDERS[0].eta;
  ...
  if (phase === "arrived") return (
    <div className="min-h-screen bg-[var(--background)] flex flex-col items-center justify-center px-6 space-y-6">
      ...
      <Link href="/customer/rate-job" className="w-full max-w-xs h-14 bg-[var(--primary)] text-white rounded-xl font-semibold flex items-center justify-center gap-2 hover:opacity-90 transition-opacity">
